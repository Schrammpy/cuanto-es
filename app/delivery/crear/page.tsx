...
export default function CrearComercio() {
  ...
  const handleSave = async () => {
    ...
    if (error) {
      alert("❌ El nombre de esta tienda ya existe o hay un error de conexión.");
    } else {
      ...
      <div className="max-w-md w-full space-y-6">
        ...
        <div className="bg-blue-50 border border-blue-100 p-5 rounded-[2rem] shadow-sm">
            ...
            <div className="space-y-4 text-slate-700 font-medium leading-tight text-[11px]">
                ...
                <div className="flex items-center gap-4 text-blue-700 font-[800]">
                    ...
                    <p>¡Listo! Generá tu propio link profesional.</p>
                </div>
            </div>
        </div>
        ...
        <div className="bg-white rounded-[2.5rem] p-6 shadow-sm border border-slate-100 space-y-6">
            <div className="space-y-3">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Información de la Tienda</p>
                <div className="bg-[#F1F5F9] p-3 rounded-2xl flex items-center border-2 border-transparent focus-within:border-blue-600 focus-within:bg-white transition-all text-slate-700">
                    ...
                    <input placeholder="Nombre de tu Tienda" className="bg-transparent w-full outline-none font-bold text-sm" onChange={e => handleNombreChange(e.target.value)} />
                </div>
                <div className="bg-slate-50 p-3 rounded-2xl flex items-center border border-slate-100 opacity-60">
                    <Globe className="w-4 h-4 text-blue-400 mr-3" />
                    <input value={form.slug ? `cuantoes.com.py/delivery/${form.slug}` : "Link automático..."} readOnly className="bg-transparent w-full outline-none font-bold text-[10px] text-blue-400 italic" />
                </div>
                <div className="bg-[#F1F5F9] p-3 rounded-2xl flex items-center border-2 border-transparent focus-within:border-blue-600 focus-within:bg-white transition-all text-slate-700">
            ...
            <div className="space-y-2">
                ...
                <div className="h-56 w-full rounded-3xl overflow-hidden border-4 border-white shadow-inner relative z-0">
                    <MapContainer center={pos} zoom={13} style={{ height: '100%', width: '100%' }}>
                        ...
                        <MapEventsHandler setPos={setPos} />
                    </MapContainer>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
                <div className="bg-white p-3 rounded-2xl flex flex-col border-2 border-slate-100 focus-within:border-blue-500 transition-all shadow-sm">
                    ...
                    <input placeholder="10.000" inputMode="numeric" className="bg-transparent w-full outline-none font-black text-sm text-slate-800" value={form.precio_base} onChange={e => setForm({...form, precio_base: formatMiles(e.target.value)})} />
                </div>
                <div className="bg-white p-3 rounded-2xl flex flex-col border-2 border-slate-100 focus-within:border-blue-500 transition-all shadow-sm">
                    <span className="text-[8px] font-black text-blue-500 uppercase mb-1">Adicional por Km</span>
                    <input placeholder="1.000" inputMode="numeric" className="bg-transparent w-full outline-none font-black text-sm text-slate-800" value={form.precio_extra_km} onChange={e => setForm({...form, precio_extra_km: formatMiles(e.target.value)})} />
                </div>
            </div>

            <button onClick={handleSave} disabled={loading} className="w-full bg-blue-600 text-white font-[900] py-5 rounded-3xl shadow-xl shadow-blue-200 active:scale-95 transition-all flex justify-center items-center gap-3 tracking-tight">
...
function MapEventsHandler({ setPos }: { setPos: (p: [number, number]) => void }) {
    // @ts-ignore
    const { useMapEvents } = require('react-leaflet');
    const map = useMapEvents({ click(e: any) { setPos([e.latlng.lat, e.latlng.lng]); map.flyTo(e.latlng, map.getZoom()); } });
    return null;
}