...
import { 
  Menu, X, Sparkles, Calculator, Wrench, ShieldAlert, Truck, 
  Map as MapIcon, ChevronDown, UserPlus, Info 
} from 'lucide-react';
...
export default function Navbar() {
  ...
  const services = [
    ...
    { name: 'Plomería y Fugas', href: '/costos/plomeria' },
    { name: 'Electricidad del Hogar', href: '/costos/electricidad' },
    { name: 'Limpieza y Fin de Obra', href: '/costos/limpieza' },
    { name: 'Albañilería y Murallas', href: '/costos/albanileria' },
  ];

  const tools = [
    { name: 'Escapadas IA', icon: <MapIcon className="w-4 h-4 text-emerald-600" />, href: '/escapadas' }, 
    ...
    { name: 'Delivery Business', icon: <Truck className="w-4 h-4 text-indigo-600" />, href: '/delivery/crear' },
  ];
        ...
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            <div className="relative group">
              ...
              <div className="absolute top-full -left-4 pt-2 hidden group-hover:block w-56 animate-in fade-in slide-in-from-top-2">
            ...
            <div className="relative group">
              ...
              <div className="absolute top-full -left-4 pt-2 hidden group-hover:block w-56 animate-in fade-in slide-in-from-top-2">
                <div className="bg-white border border-slate-100 shadow-2xl rounded-2xl p-2 space-y-1">
                  ...
                  <Link href="/multas" className="flex items-center gap-3 p-3 text-xs font-bold text-slate-600 hover:bg-red-50 hover:text-red-600 rounded-xl transition-all">
                    <ShieldAlert className="w-4 h-4 text-red-500" /> Multas Py
                  </Link>
                </div>
              </div>
            </div>

            {/* ENLACES A LAS PÁGINAS "LEGALES/B2B" */}
            <div className="w-[1px] h-6 bg-slate-200 mx-2"></div>
            
            <Link href="/metodologia" className="flex items-center gap-1.5 text-xs font-bold text-slate-500 hover:text-slate-800 transition-colors">
              <Info className="w-4 h-4" /> Metodología
            </Link>
          ...
          <div className="p-6 space-y-6 h-full flex flex-col justify-between overflow-y-auto">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2 pt-4">Herramientas</p>
                {tools.map(t => (
                ...
                <Link href="/multas" onClick={() => setIsOpen(false)} className="flex items-center gap-3 p-3 hover:bg-red-50 rounded-2xl text-sm font-bold text-slate-700 transition-colors"><ShieldAlert className="w-4 h-4 text-red-600" /> Multas de Tránsito</Link>
              </div>
            </div>

            {/* SECCIÓN INFORMATIVA MOBILE */}
            <div className="pt-6 border-t border-slate-100 space-y-3">
              <Link href="/metodologia" onClick={() => setIsOpen(false)} className="flex items-center justify-center gap-2 w-full text-slate-500 font-bold py-3 text-xs uppercase tracking-widest hover:text-slate-800 transition-colors">
                <Info className="w-4 h-4" /> Metodología de Precios
              </Link>
              <Link 
                href="/profesionales" 
